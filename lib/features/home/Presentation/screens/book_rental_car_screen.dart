...
import 'package:car_rental_app/features/home/Presentation/widgets/labeled_value_card.dart';
...
class BookRentalCarScreen extends StatefulWidget {
  const BookRentalCarScreen({super.key, required this.datePickerBloc,required this.carModel});
...
class _BookRentalCarScreenState extends State<BookRentalCarScreen> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await context.read<BookRentalCubit>().loadInfo(widget.datePickerBloc,widget.carModel);
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      CustomTextfield(
                        isEnabled:
                            context.read<BookRentalCubit>().state.name == null,
                      ...
                      PhoneTextfield(
                        onSubmit: (value) {
                          context.read<BookRentalCubit>().setPhoneNumber(value);
                        },
                        enabled:
                            context.read<BookRentalCubit>().state.phoneNumber ==
                            null,
                        initialCountry: "EG",
                            ...
                            children: [
                                ...
                                child:
                                    ...
                                    >(
                                      buildWhen: (previous, current) {
                                        return previous.currentPosition != current.currentPosition || previous.markers != current.markers;
                                      },
                                      builder: (context, state) {
                                        return GoogleMap(
                                          onMapCreated: (c) =>
                                              _pickupMapController = c,
                              ...
                              Positioned.fill(
                                child: GestureDetector(
                                  onTap: () => context.push(
                                    AppRoutes.map,
                                    extra: context.read<BookRentalCubit>(),
                                  ),
                      ...
                      BlocBuilder<BookRentalCubit, BookRentalState>(
                        buildWhen: (previous, current) => previous.pickupAddress != current.pickupAddress,
                        builder: (context, state) {
                      ...
                      BlocBuilder<BookRentalCubit, BookRentalState>(
                        buildWhen: (previous, current) {
                          ...
                          return previous.pickupDate != current.pickupDate ||
                              previous.dropOffDate != current.dropOffDate;
                        },
                        builder: (context, state) {
                          return Column(
                            children: [
                                      ...
                                      onTap: () {
                                        AdaptiveTimePicker.show(
                                          ...
                                          context
                                              ...
                                              .setPickupTime(value,widget.carModel);
                                        });
                                      },
                                    ),
                                  ),
                                    ...
                                    child: LabeledValueCard(
                                      ...
                                      onTap: () {
                                        AdaptiveTimePicker.show(
                                          ...
                                          context
                                              .read<BookRentalCubit>()
                                              .setDropOffTime(value,widget.carModel);
                                        });
                                      },
                                    ),
                                  ),
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                _SummaryRow(
                                  label: "Price per day",
                                  value: "\$${widget.carModel.pricePerDay.toStringAsFixed(0)}",
                                ),
                                const SizedBox(height: 8),
                                _SummaryRow(
                                  label: "Duration",
                                  value: "${state.rentalDuration} ${state.isHourly ? 'hours' : 'days'}",
                                ),
                                const SizedBox(height: 8),
                                _SummaryRow(
                                  label: "Subtotal",
                                ...
                                _SummaryRow(
                                  label: "Service fee",
                                  value: "\$${(state.subtotoal! * state.serviceFeeRate).toStringAsFixed(0)}",
                                ),
                                const SizedBox(height: 8),
                                _SummaryRow(label: "Tax", value: "\$${(state.subtotoal! * state.taxRate).toStringAsFixed(0)}"),
                                const SizedBox(height: 12),
                      ...
                      SizedBox(
                        ...
                        child: AdaptiveButton(
                          ...
                          onPressed: () async {
                            ...
                            print(formValid ? "Form is valid" : "not valid");
                            final s = context.read<BookRentalCubit>().state;
                            final hasName = (s.name?.trim().isNotEmpty ?? false) || _nameController.text.trim().isNotEmpty;
                            final hasPhone = (s.phoneNumber?.trim().isNotEmpty ?? false);
                            final hasAddress = s.pickupAddress != null && s.pickupAddress!.trim().isNotEmpty;
                            final hasDates = s.pickupDate != null && s.dropOffDate != null;
                            if (!formValid || !hasName || !hasPhone || !hasAddress || !hasDates) {
                              await DialogueService.showAdaptiveAlertDialog(
                                context,
                                title: "Incomplete form",
                                content: "Please enter name, phone, pickup address, and select start and end.",
                                actions: [
                                  AlertAction(title: "OK", onPressed: () => Navigator.of(context).pop(), style: AlertActionStyle.primary),
                                ],
                              );
                              return;
                            }
                            await context.read<BookRentalCubit>().bookRentalCar(widget.carModel);
                            await context.read<BookRentalCubit>().saveUserInfo(name: s.name,phoneNumber: s.phoneNumber);

                            //show success dialog
                            await DialogueService.showAdaptiveAlertDialog(
                                context,
                                title: "Success",
                                content: "Your booking has been submitted. you will be notified if owner accepts your request.",
                                actions: [
                                  AlertAction(title: "OK", onPressed: () => context.pop(), style: AlertActionStyle.primary),
                                ],
                              );
                          },
